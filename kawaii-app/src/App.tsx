...
import { Heart, Download, X, Sparkles, Image as ImageIcon, Loader2, Key } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const savedKey = localStorage.getItem('kawaii_gemini_api_key');
  ...
  const handleSaveApiKey = () => {
    ...
    localStorage.setItem('kawaii_gemini_api_key', apiKeyInput.trim());
  ...
  const handleRemoveApiKey = () => {
    localStorage.removeItem('kawaii_gemini_api_key');
    setApiKey('');
    setShowApiKeyScreen(false);
  };
  ...
  const handleDownload = async (url: string, filename: string) => {
    try {
      ...
      // Fallback for data URLs or cross-origin issues
      const link = document.createElement('a');
      link.href = url;
      link.download = filename;
      link.target = '_blank';
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Save generated wallpapers to local storage (keep only AI generated ones)
      ...
      if (err?.message === 'NO_API_KEY' || err?.message?.includes('API_KEY') || err?.message?.includes('not found')) {
        setShowApiKeyScreen(true);
        setError('Chave de API inválida ou em falta. Por favor insere a tua chave.');
      } else {
      ...
      <AnimatePresence>
        {showApiKeyScreen && (
          <motion.div
            ...
            onClick={() => setShowApiKeyScreen(false)}
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full border border-pastel-pink/20"
            ...
            >
             ...
             <p className="text-gray-500 text-center text-sm mb-6">
              ...
              {apiKey && (
                <div className="mb-4 p-3 bg-green-50 rounded-2xl border border-green-200 flex items-center justify-between">
                  <span className="text-green-700 text-sm font-medium">✅ Chave guardada: ...{apiKey.slice(-6)}</span>
                  <button onClick={handleRemoveApiKey} className="text-red-400 text-xs hover:text-red-600 font-semibold">Remover</button>
                </div>
              )}
              ...
              <input
                ...
                placeholder="Cole aqui a tua chave: AIza..."
                className="w-full p-4 rounded-2xl bg-gray-50 border-2 border-pastel-pink/30 focus:border-pink-400 focus:ring-4 focus:ring-pink-100 outline-none transition-all text-gray-700 mb-4 font-mono text-sm"
              />

              <button
                onClick={handleSaveApiKey}
                disabled={!apiKeyInput.trim()}
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-pink-400 to-purple-400 text-white font-bold text-lg shadow-lg disabled:opacity-40 disabled:cursor-not-allowed hover:shadow-pink-300 transition-all"
              >
                Guardar Chave
              </button>
            </motion.div>
        ...
        {activeTab === 'generate' ? (
          ...
          >
            <div className="bg-white rounded-3xl shadow-xl shadow-pink-100/50 p-8 border border-pastel-pink/20">
              ...
              {!apiKey && (
                <div className="mb-6 p-4 bg-pink-50 rounded-2xl border border-pink-200 text-center">
                  <p className="text-pink-600 text-sm font-medium mb-2">⚠️ Precisas de adicionar a tua Gemini API Key primeiro!</p>
                  <button
                    onClick={() => setShowApiKeyScreen(true)}
                    className="px-4 py-2 bg-pink-400 text-white rounded-xl text-sm font-bold hover:bg-pink-500 transition-colors"
                  >
                    Adicionar API Key
                  </button>
                </div>
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                    <div className="flex justify-between items-end">
                      ...
                      >
                        <Heart 
                          className={`w-5 h-5 ${favorites.includes(wallpaper.id) ? 'fill-pink-400 text-pink-400' : 'text-white'}`} 
                        />
                      </button>
                    </div>
                  </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex items-center justify-between">
                <div className="flex-1">
                  {selectedImage.prompt && (
                    <p className="text-white/80 text-sm line-clamp-2 mb-2">{selectedImage.prompt}</p>
                  )}
                  <span className="text-white text-sm font-medium bg-white/20 backdrop-blur-md px-3 py-1 rounded-full">
                    {selectedImage.category || 'Aesthetic'}
                  </span>
                </div>
                ...
                <div className="flex gap-3 ml-4">
                  ...
                  >
                    <Heart 
                      className={`w-6 h-6 ${favorites.includes(selectedImage.id) ? 'fill-pink-400 text-pink-400' : 'text-white'}`} 
                    />
                  </button>
                  <button 
                    onClick={() => handleDownload(selectedImage.url, `kawaii-wall-${selectedImage.id}.png`)}
                    className="p-4 rounded-full bg-gradient-to-r from-pink-400 to-purple-400 text-white shadow-lg hover:shadow-pink-400/50 transition-all hover:scale-105"
                  >
                    <Download className="w-6 h-6" />
                  </button>
                </div>
  ...
  return (
    <button 
      ...
      className={`flex flex-col items-center gap-1 transition-all ${
        isActive ? (isPrimary ? 'text-pink-500' : 'text-gray-800') : 'text-gray-400 hover:text-gray-600'
      }`}
    >
      <div className={`p-2 rounded-2xl transition-all ${
        isActive 
          ? (isPrimary ? 'bg-pink-100 shadow-inner' : 'bg-gray-100') 
          : 'hover:bg-gray-50'
      }`}>
        {icon}
      </div>
